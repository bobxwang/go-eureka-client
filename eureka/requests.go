...
func formatInstanceId(app, ip string, port int) string {
	return ip + ":" + app + ":" + strconv.Itoa(port)
}
...
func NewInstanceInfo(hostName, app, ip string, port int, ttl uint, isSsl bool) *InstanceInfo {
	...
	instanceInfo := &InstanceInfo{
		InstanceId:     formatInstanceId(app, ip, port),
		HostName:       hostName,
		App:            app,
		IpAddr:         ip,
		Status:         UP,
	...
	instanceInfo.StatusPageUrl = protocol + "://" + hostName + stringPort + "/actuator/info"
	instanceInfo.HealthCheckUrl = protocol + "://" + hostName + stringPort + "/actuator/health"
	return instanceInfo
}
